...
This proposal covers only the permissions associated with the DAC Chair and Member roles, not the Researcher role (creating / editing a data access application). 
...
* **Authz**: modify study endpoints to allow list / create / delete of dac authorizations, ensuring that only DAC members for the specific study can view authorizations, and only DAC chairs can add / delete. Users can GET their own authorizations.  
...
In order to approve a data access request, the DACO Portal makes a POST to `POST /study/study_id/dac_authorizations` with the Bearer token of the authenticated user. Authz will determine if the user (the DAC member) has the appropriate role for the study based on the `dac_id` in the study registration. 
...
Similarly, a DAC Chair can revoke a DAC authorization using a DELETE to `/study/study_id/dac_authorizations/`. 
